function [tracks,traj]=track2d_manualfit(folderout,FileName,data,maxdist,lmin,flag_pred,npriormax,flag_conf,test)

% 03/2019 - Thomas Basset 
% 01/2020 - "Optimisation" David Dumont
% adapted for tracking on 2D pictures -- IMFT/MPB by O. Liot march 2023
% Track particles on 2D images
...
%raw data
tracks=zeros(length(horzcat(data.X)),6);
size(tracks)
size(tracks(:,1))
tracks(:,1)=horzcat(data.frame)'; %frame number
tracks(:,2)=horzcat(data.X)'; %X position
tracks(:,3)=horzcat(data.Y)'; %Y position
tracks(:,4)=0; %trajectory index
tracks(:,5)=1; %availability of particles (0 unavailable, 1 available, 2 conflict)
tracks(:,6)=(1:size(tracks,1))'; %element index

%number of particles in first frame = number of active trajectories
ind_act=find(tracks(:,1)==min(tracks(:,1)));
tracks(ind_act,4)=ind_act;
tracks(ind_act,5)=0;
ntraj=max(ind_act);
...
for kf=min(tracks(:,1))+1:max(tracks(:,1)) %for each frame
    ...
    for kp=1:length(ind_act) %for each particle in frame kf-1
        ...
        actnum=tracks(ind_act(kp),4); %trajectory index
...
traj = struct('ntraj',cell(1,numel(i)),'L',[],'frames',[],'x',[],'y',[],'nmatch',[]);
for kk = 1 :length(ftrackselec)
    ...
    traj(kk).nmatch = tracks_array(deb:deb+ll-1,6);  % ki previously
...
for k=1:length(fields)
    fieldk = fields{k};
    data_to_save = vertcat(traj.(fieldk));
    if isempty(data_to_save)
        data_to_save = NaN(1,1);  % <-- dataset minimal, valeur unique
    end
    h5create(FileSaveName, ['/' fieldk], [length(data_to_save) 1], ...
             'ChunkSize', [length(data_to_save) 1], 'Deflate', 9);
    h5write(FileSaveName, ['/' fieldk], data_to_save);
end
